...
excluded_words = ['the', 'a', 'an', 'and', 'or', 'but', 'for', 'with', 'in', 'on', 'at', 'to', 'from', 't','ve']  # Palabras a excluir en inglés
...
with open("endpoint.csv", mode='r') as file:
    ...
    for row in reader:
        quote = row[1]
        words = re.findall(r"\b(?:\w+(?:'\w+)?|\w+)\b", quote)  # Extraer palabras completas con apóstrofes
